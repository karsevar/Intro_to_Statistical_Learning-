...
mean((College.test$Apps - College.pred)^2)
#The test RSS is 1108531
...
ridge.fit <- cv.glmnet(train.mat, College.train$Apps, alpha = 0, lambda = grid, thresh = 1e-12)
best.lambda <- ridge.fit$lambda.min
best.lambda# The best lambda was recorded at 0.01149757
ridge.pred <- predict(ridge.fit, newx = test.mat, s = best.lambda)
mean((College.test$Apps - ridge.pred)^2)
#The test RSS is 1108512, which is almost the same as the linear regression model. 

#(d) 
lasso.fit <- cv.glmnet(train.mat, College.train$Apps, alpha = 1, lambda = grid, thresh = 1e-12)
best.lambda <- lasso.fit$lambda.min
lasso.pred <- predict(lasso.fit, newx = test.mat, s = best.lambda)
mean((College.test$Apps - lasso.pred)^2)
#The RSS value was calculated at 1028718, which is a little higher than that calculated through the best subset method and the ridge regression method.
...
# The pcr method calculated the MSE at 1904928. Will need to see if changing the ncomp argument will change the following prediction. 

pcr.pred <- predict(pcr.fit, College.test, ncomp = 10)
mean((pcr.pred - College.test$Apps)^2)
#the value is now calculated at 1505718 (Which is still much higher than the other methods).

MSE <- NA
for(i in 1:17){
	pcr.pred <- predict(pcr.fit, College.test, ncomp = i)
	MSE[i] <- mean((pcr.pred - College.test$Apps)^2)
}	
	
plot(x = c(1:17), y = MSE, ylab = "Mean Squared Error", xlab = "ncomp value", type = "l")# The following graphic illustrates the most optimum ncomp value. I believe that the best value would be ncomp = 10.
pcr.pred <- predict(pcr.fit, College.test, ncomp = 10)
mean((pcr.pred - College.test$Apps)^2)
#the MSE value is 1505718 which is still a little higher than the other methods. 
...
MSE <- NA
for(i in 1:17){
	pls.pred <- predict(pls.fit, College.test, ncomp = i)
	MSE[i] <- mean((pls.pred-College.test$Apps)^2)
}
plot(x = c(1:17), y = MSE, ylab = "MSE", xlab = "ncomp value", type = "l")# According to this plot the smallest MSE might be M = 6. Will look into this through the which.min() function.
which.min(MSE)# of course the smallest will be in the sixteen mark. I might have to ignore this answer. 
pls.pred <- predict(pls.fit, College.test, ncomp = 6)
mean((pls.pred - College.test$Apps)^2)# the MSE is calculated at 112189 (which is a little better than the PCR method).

#(g) 
#Will work on this question later.

#10 Answer obtained from Anaboughi: 
#(a) 
set.seed(1)
p <- 20 
n <- 1000
...
#(c) 
best.fit <- regsubsets(y ~ x, data = data.frame(x = x.train, y = y.train), nvmax = p)
val.errors <- rep(NA, p)
...
best.fit <- regsubsets(crim ~., data = Boston)
summary(best.fit)
#According to the best subset method the most statistically important variable is rad (index of accessibility of radial highways), followed by zn (the proportion of residential land zoned for lots over 25,000 sq.ft.) and medv (median value of owner occupied homes in 1000). 
names(summary(best.fit))
best.fit.sum <- summary(best.fit)
best.fit.cp <- which.min(best.fit.sum$cp)
#c_p statistic records eight variables will create a statistically powerful model.
best.fit.bic <- which.min(best.fit.sum$bic)
#The bic statistic calculates a model containing 3 variables as statistically significant. 
best.fit.adjr2 <- which.max(best.fit.sum$adjr2)
#The adjusted R squared statistically powerful model was calculated at 8 variables. 

#set cross validation:
train <- sample(nrow(Boston)/2, replace = FALSE)
...
best.pre <-predict.regsubsets(best.fit, Boston.test, id = 13)
mean((Boston$crim[test] - pred)^2)# I can't seem to get this function to work properly. will have to go back to this operation later.

#Anaboughi's solution:
val.errors <- rep(NA, 13)
test.mat <- Boston[test,-1]
coefi <- NA
for(i in 1:13){
	coefi <- coef(best.fit, id = i)
	pred <- test.mat[,names(coefi)]%*%coefi
	val.errors[i] <- mean((Boston.test$crim-pred)^2)
}# this function variant doesn't work as well. Will need to look into what the problem is. 
...
# the rmse.cv value was calculated at 0.9418

#Lasso method:
set.seed(1)
train <- sample(1:nrow(Boston), nrow(Boston)/2)
test <- (-train)
Boston.x.test <- model.matrix(crim ~. -1, data = Boston[test,])
Boston.y.test <- Boston$crim[test]
Boston.y.train <- Boston$crim[train]
Boston.x.train <- model.matrix(crim ~. -1, data = Boston[train,])
lasso.fit <- glmnet(Boston.x.train, Boston.y.train, alpha = 1, lambda = grid)
plot(lasso.fit)

set.seed(1) 
cv.out <- cv.glmnet(Boston.x.train, Boston.y.train, alpha = 1)
plot(cv.out)
bestlam <- cv.out$lambda.min
lasso.pred <- predict(lasso.fit, s = bestlam, newx = Boston.x.test)
mean((lasso.pred - Boston.y.test)^2)
#The MSE value was calculated at 38.31093

#ridge regression:
set.seed(1)
cv.out <- cv.glmnet(Boston.x.train, Boston.y.train, alpha = 0)
plot(cv.out)
bestlam <- cv.out$lambda.min
bestlam
plot(cv.out)
